...
nu texteffect("v", 2, u, function(argstr) { //(V)viggle, 2, xint yint
	//log(argstr);
	wiggle.x = easemult(ac_sin, global.count+i, 120, real(string_copy(argstr, 1, 1)));
	wiggle.y = easemult(ac_cos, global.count+i, 120, real(string_copy(argstr, 2, 1)));
...
nu texteffect("b", 0, function(argstr) { //(B)olding, 0
	log("bold bolder yet boulder");
	bolding = !bolding;
});